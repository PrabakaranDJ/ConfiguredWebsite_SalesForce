...
import com.salesforce.util.Xls_Reader;
...
public class Page {
	...
	public static Xls_Reader xls1= new Xls_Reader(System.getProperty("user.dir")+"//src//com//salesforce//xls//TestCases.xlsx");
	...
	public Page(){
		...
		try{
			...
			// OR
			fs = new FileInputStream(System.getProperty("user.dir")+"\\src\\com\\salesforce\\config\\OR.properties");
			OR.load(fs);
			}catch(Exception e){
				// error
				return;
	...
	public void select(String xpathKey, String text){
		try{
		driver.findElement(By.xpath(OR.getProperty(xpathKey))).sendKeys(text);
		}catch(Exception e){
			// report error
			...
			// select
	// verification
/*	public boolean isElementPresent(By linkText){
		try{
			driver.findElement(By.xpath(OR.getProperty(linkText)));
		}catch(Exception e){
			return false;
		}
